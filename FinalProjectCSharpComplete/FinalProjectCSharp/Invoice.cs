...
namespace FinalProjectCSharp
{
    ...
    public partial class Invoice : Form
    {
        ...
        private DataTable GetPartAndLabourList()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT quantity AS 'Qty/Hrs', part_number AS 'Part#', part_name AS 'Part Name', price AS 'Price', discount AS 'Discount', hst AS 'HST(13%)', VAT, total AS 'Total' FROM tbl_invoice_parts_labour WHERE invoice_number LIKE @invoiceNo", con))
                {
                    con.Open();
        ...
        private void Invoice_Load(object sender, EventArgs e)
        {
            ...
            txt_Date.Text = todaysDate.Date.ToString("d");
            ...
            SqlCommand cmd = new SqlCommand("SELECT invoice_number FROM tbl_invoice", con);
            ...
            if (DraftCount == 0)
            {
                while (myReader.Read())
                {
                    txt_InvoiceNo.Text = (myReader["invoice_number"].ToString());
                }
         ...
         /*    if(DraftCount ==0)
            ...
            if (DraftCount == 1)
            {
                ...
                txt_InvoiceNo.Text = InvoiceDraft.SetValueForInvoiceNo;
                txt_Comments.Text = InvoiceDraft.SetValueForComments;
                txt_Date.Text = todaysDate.Date.ToString("d");
                // cb_TaxExempt.Text = InvoiceDraft.se;
                txt_FirstName.Text = InvoiceDraft.SetValueForFirstName;
                txt_LastName.Text = InvoiceDraft.SetValueForLastName;
                txt_PhoneNumber.Text = InvoiceDraft.SetValueForPhoneNumber;
                txt_Year.Text = InvoiceDraft.SetValueForYear;
                txt_Make.Text = InvoiceDraft.SetValueForMake;
                txt_Model.Text = InvoiceDraft.SetValueForModel;
                txt_Mileage.Text = InvoiceDraft.SetValueForMileage;
                //this.Refresh();
        ...
        {
            cmd = new SqlCommand("SELECT quantity AS 'Qty/Hrs', part_number AS 'Part#', part_name AS 'Part Name', price AS 'Price', discount AS 'Discount', hst AS 'HST(13%)', VAT, total AS 'Total' FROM tbl_invoice_parts_labour WHERE invoice_number LIKE @invoiceNo", con);
            con.Open();
            cmd.Parameters.AddWithValue("@invoiceNo", txt_InvoiceNo.Text);
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            else if (!System.Text.RegularExpressions.Regex.IsMatch(txt_InvoiceNo.Text, "^[0-9 ]"))
            {
                txt_InvoiceNo.Clear();
            ...
            if (txt_InvoiceNo.Text != "" && txt_Date.Text != "" && txt_Model.Text != "" && txt_Make.Text != "" && txt_Year.Text != "" && txt_Mileage.Text != "" && txt_FirstName.Text != "" && txt_LastName.Text != "" && txt_PhoneNumber.Text != "" && txt_Total.Text != "0.00")
            {
                cmd = new SqlCommand("UPDATE tbl_invoice SET date_saved = @date, first_Name = @firstName, last_Name = @lastName, phone_number = @phNumber, vehicle_year = @year, vehicle_make = @make, vehicle_model = @model, vehicle_mileage =  @mileage WHERE invoice_number = @invoiceNumber", con);
                con.Open();
                cmd.Parameters.AddWithValue("@invoiceNumber", txt_InvoiceNo.Text);
        ...
        private void btn_Exit_Click(object sender, EventArgs e)
        {
            ...
            string title = "Exit Invoice";
            MessageBoxButtons buttons = MessageBoxButtons.YesNo;
            DialogResult result = MessageBox.Show(message, title, buttons);
            if (result == DialogResult.Yes)
            {
                cmd = new SqlCommand("DELETE FROM tbl_invoice_parts_labour WHERE invoice_number = @invoiceNum", con);
                con.Open();
                cmd.Parameters.AddWithValue("@invoiceNum", txt_InvoiceNo.Text);
                cmd.ExecuteNonQuery();
                con.Close();

                cmd = new SqlCommand("DELETE FROM tbl_invoice WHERE invoice_number = @invoiceNum", con);
                con.Open();
                cmd.Parameters.AddWithValue("@invoiceNum", txt_InvoiceNo.Text);
                cmd.ExecuteNonQuery();
                con.Close();
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            cusSearch.Show();
            this.Hide();
            InvoiceCustomerSearch.displayInfo = 0;
            DraftCount = 0;
        }
        ...
        private void btn_Submit_Click(object sender, EventArgs e)
        {
            ...
            if (txt_InvoiceNo.Text != "" && txt_Date.Text != "" && txt_Model.Text != "" && txt_Make.Text != "" && txt_Year.Text != "" && txt_Mileage.Text != "" && txt_FirstName.Text != "" && txt_LastName.Text != "" && txt_PhoneNumber.Text != "" && txt_QuantityHours.Text != "" && txt_PartNumber.Text != "" && txt_PartName.Text != "" && txt_Price.Text != "" && txt_Discount.Text != "")
            {
                ...
                double HST = Math.Round(((price * quantityHours) * 0.13), 2);
                double Total = Math.Round(((price * quantityHours) - ((price * quantityHours) * (discount / 100))) + HST, 2);

                cmd = new SqlCommand("INSERT INTO tbl_invoice_parts_labour (invoice_number, quantity, part_number, part_name, price, discount, hst, VAT, total) VALUES (@invoiceNo, @quantity, @partNo, @partName, @price, @discount, @hst, @VAT, @total)", con);
                con.Open();
                cmd.Parameters.AddWithValue("@invoiceNo", txt_InvoiceNo.Text);
        ...
        private void btn_SaveAsDraft_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand("INSERT INTO tbl_invoice_draft(invoice_number, date, first_Name, last_Name, phone_number , v_year , v_make , v_model , mileage, comments, tax_exempt) VALUES  (@invoiceNumber, @date, @firstName, @lastName, @phNumber, @year, @make, @model, @mileage, @comments, @taxExempt)", con);
            con.Open();
            cmd.Parameters.AddWithValue("@invoiceNumber", txt_InvoiceNo.Text);
            cmd.Parameters.AddWithValue("@date", txt_Date.Text);
            cmd.Parameters.AddWithValue("@firstName", txt_FirstName.Text);
